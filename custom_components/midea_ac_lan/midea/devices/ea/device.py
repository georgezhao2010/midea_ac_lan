...
class MideaEADevice(MiedaDevice):
    def __init__(
        ...
        self._progress = ["Idle", "Delay", "Cooking", "Keep warm"]

    def software_version(self):
        if self._software_version is not None:
            return self._software_version
        self._software_version = 1
        if self._sub_type in [0, 1, 3, 4, 5, 6]:
            self._software_version = 0
        elif self._sub_type in [39, 40, 41, 42, 43, 44, 45, 47, 48]:
            self._software_version = 3
        return self._software_version

    def build_query(self):
        return [MessageQuery(self._device_protocol_version, self.software_version)]
            ...
            if hasattr(message, status.value):
                ...
                if status == DeviceAttributes.progress:
                    if value < len(self._progress):
                        self._attributes[status] = self._progress[getattr(message, status.value)]
                    else:
                        self._attributes[status] = "Unknown"
                elif status == DeviceAttributes.mode:
                    if value == 198:
                        self._attributes[status] = "keep_warm"
                    elif value == 192:
                        self._attributes[status] = "clean"
                    elif value >= 199:
                        self._attributes[status] = "diy"
                    elif value < len(self._mode_list):
                        self._attributes[status] = self._mode_list[getattr(message, status.value)]
                    else:
                        self._attributes[status] = "unknown"
                else:
